...
let DemoObjects = function () {
  ...
  this.display = () => {
    ...
    m.translate(0, 1, -0.4);
    m.scale(0.04);
    renderList.mCube().move(-4.5, 4.5, 0).turnY(time).color([1, 1, 1]).textureView(window.textures[1].lookupImageByID(1)).textureAtlas(window.textures[1]);
    renderList.mQuad().move(-1.5, 4.5, 0).turnY(time).color([1, 1, 1]).textureView(window.textures[0].lookupImageByID(1)).textureAtlas(window.textures[0]);;
