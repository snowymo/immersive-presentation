...
let DemoObjects = function () {
  ...
  this.display = () => {
    ...
    renderList
      ...
      .move(-1.5, 1.5, 0)
      .turnY(time)
      .color([1, 1, 1]).textureView(window.textures[4].lookupImageByID(1)).textureAtlas(window.textures[4]);
    renderList.mTorus().move(1.5, 1.5, 0).turnY(time).color([1, 1, 1]).textureView(window.textures[0].lookupImageByID(1)).textureAtlas(window.textures[0]);
