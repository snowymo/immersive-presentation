...
export function createAudioSource(options) {
  ...
  return fetch(options.url)
    ...
    .then((decodedBuffer) => {
      let bufferSource = createBufferSource(source, decodedBuffer, analyser);
...
function getLoudnessScale(analyser) {
  ...
  let sum = 0;
  for (let i = 0; i < fftBuffer.length; ++i) sum += fftBuffer[i] * fftBuffer[i];
...
export function updateAudioSources(frame, refSpace) {
  ...
  for (let source of audioSources) {
    if (source.draggingInput) {
      let draggingPose = frame.getPose(
        source.draggingInput.targetRaySpace,
        refSpace
      );
      if (draggingPose) {
        ...
        mat4.multiply(
          tmpMatrix,
          draggingPose.transform.matrix,
          source.draggingTransform
        );
